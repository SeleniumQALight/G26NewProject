...
public class ExcelDriver {
	...
	public static Map getMultipleData(String dataFileName, String sheetName, int columnNumber) throws IOException {
		//метод получает по имени листа и файлу колонки
		...
		HSSFSheet sheet = wb.getSheet(sheetName); //sheetName - сюда передаем

		// Get number of data values
        int dataSize = sheet.getPhysicalNumberOfRows() - 1; //этот метод дает то что заполнено, сколько физически строк занятых, тут менять цифру в зависимости от строк в файле
    ...
    /*
      ...
      * data values in the test.
      */
    public static Map getData(String dataFileName, String sheetName) throws IOException { // берет внутри зашитое что 0 колонка это key, 1-value
        Map<String, String> testData = new HashMap<String, String>();
        // Create stream for reading from file
        InputStream input = new FileInputStream(dataFileName);
        // Get Excel WorkBook from input stream
