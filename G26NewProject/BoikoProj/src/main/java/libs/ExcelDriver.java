...
public class ExcelDriver {
	...
	public static Map getMultipleData(String dataFileName, String sheetName, int columnNumber) throws IOException {
        ...
        int dataSize = sheet.getPhysicalNumberOfRows() - 1; //-1 because ignore first row
		...
		for (int k = 1; k < (dataSize + 1); k++) {
			...
			HSSFCell valueCell = sheet.getRow(k).getCell(columnNumber); //Take row = k and value from columnNumber
			valueCell.setCellType(HSSFCell.CELL_TYPE_STRING); //Transform vale to String
			testData.put(keyCell.getStringCellValue(), valueCell.getStringCellValue()); //Put key and cell values to Map
		}
    ...
    public static Map getData(String dataFileName, String sheetName) throws IOException {
        ...
        for (int k = 1; k < (dataSize + 1); k++) {
            HSSFCell keyCell = sheet.getRow(k).getCell(0);
            HSSFCell valueCell = sheet.getRow(k).getCell(1); //Always take vales for key from Column 1
