...
public class ExcelDriver {
    ...
    /*
      ...
      * data values in the test.
      */
    public static Map getData(String dataFileName, String sheetName) throws IOException { // статик используем, чтобы обращаться к методу по имени и не создавать объект
        Map<String, String> testData = new HashMap<String, String>();
        // Create stream for reading from file
        InputStream input = new FileInputStream(dataFileName);
        // Get Excel WorkBook from input stream
        ...
        // Get number of data values
        int dataSize = sheet.getPhysicalNumberOfRows() - 1; // -1 для игнора первой (одной) строки
        // Look over the table and put key-value pairs into the Map collection
        for (int k = 1; k < (dataSize + 1); k++) {
            HSSFCell keyCell = sheet.getRow(k).getCell(0);
            HSSFCell valueCell = sheet.getRow(k).getCell(1);
            valueCell.setCellType(HSSFCell.CELL_TYPE_STRING);
            testData.put(keyCell.getStringCellValue(), valueCell.getStringCellValue()); // записываем в результирующий data set пару key - value
        }

        input.close(); // делаем закрытие файла
        return testData; // фозврящаем Map пару
    }
