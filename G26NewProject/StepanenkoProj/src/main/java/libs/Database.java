...
public class Database {
   ...
   static Logger log = Logger.getLogger(Database.class);
    ...
    public Database(String db, String driver) throws IOException, ClassNotFoundException, SQLException {
        //идет в config.properties и берет url
        url=getCfgValue(db);
        
        // Load driver for JDBC class
        //получить класс для работы с драйвером ДБ
        //берет в сonfig.properties имя драйвера: MySQL=com.mysql.jdbc.Driver
...
**/
    /**
     *
     * метод изменения ДБ: умеет select, update, delete
     */
    public int changeBD (String query) throws SQLException {
        Statement stm = connection.createStatement();
        /**
         * метод апдейтает базу
         * в переменную int effectedRows = записываем кол-во измененных строк
         */
        int effectedRows = stm.executeUpdate(query);
        stm.close();
        return  effectedRows;
